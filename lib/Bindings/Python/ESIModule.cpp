...
public:
  SystemAssembler(MlirContext cCtxt) : ctxt(unwrap(cCtxt)) {
...
void circt::python::populateDialectESISubmodule(py::module &m) {
  ...
  py::class_<SystemAssembler>(m, "SystemAssembler")
      .def(py::init<MlirContext>())
      .def("load_mlir", &SystemAssembler::loadMlir,
           "Load an MLIR assembly file.")
      .def("get", &SystemAssembler::get, "Get the top level module op.")
      .def("lookup", &SystemAssembler::lookup,
